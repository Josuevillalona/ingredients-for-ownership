/**
 * FDC API Foods Route
 * Provides detailed food information by FDC ID
 */
...
export async function GET(
  ...
  try {
    ...
    const format = searchParams.get('format') as 'abridged' | 'full' || 'abridged';
    const nutrients = searchParams.get('nutrients')?.split(',').map(n => parseInt(n)).filter(n => !isNaN(n));

    // Fetch detailed food information
    const foodDetail = await fdcService.getFood(fdcId, format);
    ...
    // Convert to application FoodItem format with color assignment
    // Note: Using 'temp-coach-id' as coachId since this is for search purposes
    const foodItem = await fdcService.convertToFoodItem(foodDetail, 'temp-coach-id');

    return NextResponse.json({
      success: true,
      data: {
        fdcDetail: foodDetail,
        foodItem: foodItem
      },
      meta: {
        fdcId: fdcId,
    ...
    console.error('FDC food detail error:', error);
